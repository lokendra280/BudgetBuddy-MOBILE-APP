...
${buildFinancialContext()}''';
  ...
  static Future<String> sendMessage(List<ChatMessage> history) async {
    ...
    final contents = history.map((m) {
      final old = m
          .toApiMap(); // {'role': 'user'|'assistant', 'content': '...'}
      final role = old['role'] == 'assistant' ? 'model' : 'user';
      return {
        'role': role,
        'parts': [
          {'text': old['content']},
    ...
    final response = await http
        .post(
          ...
          headers: {
            ...
            'x-goog-api-key': _apiKey, // Gemini's auth header, not Bearer
          },
          body: jsonEncode({
            'contents': contents,
    ...
    if (response.statusCode == 200) {
      ...
      if (candidates == null || candidates.isEmpty) {
        throw Exception(
          'Gemini returned no response — the message may have been blocked by safety filters.',
        );
      }
      ...
      throw Exception('Gemini rate limit reached — try again in a moment.');
    } else {
